...
Rails has powerful support for routing arbitrary URLs to actions. Here, you don't get groups of routes automatically generated by resourceful routing. Instead, you set up each route separately within your application.
...
[incoming path of /books/1/1 will be dispatched to the show action of the BooksController. params[:id] will be "1", and params[:author_id] will be "1".]

-> Static Segments
You can specify static segments when creating a route by not prepending a colon to a segment:
[respond to paths such as /books/1/with_author/1. In this case, params would be { controller: 'books', action: 'show', id: '1', author_id: '1' }.]

->The Query String
The params will also include any parameters from the query string. For example, with this route:

[incoming path of /books/1?/author_id=1 , dispatch to the show action of the Bookscontroller. params will be { controller: 'books', action: 'show', id: '1', author_id: '1'}]

->Naming Routes
You can specify a name for any route using the :as op
...
--match 'books', to: 'books#show', via: [:get, :post]

You can match all verbs to a particular route using via: :all:

--match 'books', to: 'books#show', via: :all

->Segment constraints
 we can use the :constraints option to enforce a format for a dynamic segment:
 make data vaildation easy like-

 get '/:id', to:'books#show', constraints: { id: /\d+/ }
 [If it doesn’t pass the integer test, we don’t hit the action.only id integer then hit the action.]


-> Route Globbing and Wildcard Segments
Using a wildcard syntax (*) when defining routes in routes.rb, we can slurp up URL segments and make them available in our controller params.

e.g->
 get '/:id/*abc', to: 'books#show'
 [after id we write anything will be in store in params[:abc]]

 now, in books controller in show action we will add
 abc = params[:abc]
 now our show action will work correctly.
...
e.g->
 ...
 /books/new -> authors#new
 /books/:id/edit  -> authors#edit
 /books/:id  -> authors#show
 /books/:id  -> authors#update
 /books/:id  -> authors#destroy

 -->Overriding the Named Route Helpers
  The :as option lets you override the normal naming for the named route helpers. For example:
 resources :books, as: 'guides'
 [heplers will be generated using guide now-> guides_path, etc]
...
edit_book GET    /books/:id/change(.:format) 
new_book GET    /books/make(.:format)
...
e.g->
  ...
  [only routes will be created for new and edit]

  e.g->
    resources :books, except: :destroy

    [all routes will be created except for destroy.]
...
books      GET    /guides(.:format)                                                                                 books#index
           POST   /guides(.:format)                                                                                 books#create
new_book   GET    /guides/make(.:format)                                                                            books#new
edit_book  GET    /guides/:id/change(.:format)                                                                      books#edit
book       GET    /guides/:id(.:format)                                                                             books#show
           PATCH  /guides/:id(.:format)                                                                             books#update
           PUT    /guides/:id(.:format)                                                                             books#update
           DELETE /guides/:id(.:format)                                                                             books#destroy
...
edit_book GET    /books/:identifiers/edit(.:format)                                                                books#edit
book      GET    /books/:identifiers(.:format)                                                                     books#show
          PATCH  /books/:identifiers(.:format)                                                                     books#update
          PUT    /books/:identifiers(.:format)                                                                     books#update
          DELETE /books/:identifiers(.:format) 

-----> Breaking up very large route file into multiple small ones:
when we working on multiple files so that we can have thousands of routes so to manage them we can just make a folder for that controller inside
config/routes/file_name.rb
then, include that file in our routes.rb
e.g->
we store all routes of book controller in book.rb inside config/routes
